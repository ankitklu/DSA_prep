package Contests;
import java.util.*;
public class Minimum_Cost_Path_with_Alternating_Directions_II {
...
class Solution {
    ...
    public long minCost(int m, int n, int[][] waitCost) {
        ...
        return 1L+ dfs(0, 0, 0, m, n, waitCost, memo);
    }

    private long dfs(int r, int c, int parity, int m, int n, int[][] wait, long[][][] memo) {
        if (r == m - 1 && c == n - 1){
            return 0;
        }         

        if (memo[r][c][parity] != -1)
            return memo[r][c][parity];

        long best = INF;

        if (parity == 0) {
            if (r + 1 < m) {       
                long cost = (long) (r + 2) * (c + 1) + dfs(r + 1, c, 1, m, n, wait, memo);
                best = Math.min(best, cost);
            }
            if (c + 1 < n) {     
                long cost = (long) (r + 1) * (c + 2) + dfs(r, c + 1, 1, m, n, wait, memo);
                best = Math.min(best, cost);
            }
        } 
        else{
            best = wait[r][c] + dfs(r, c, 0, m, n, wait, memo);
        }
        return memo[r][c][parity] = best;
    }
}
